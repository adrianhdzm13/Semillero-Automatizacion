...
public class ClaseBase {
	...
	// METODO PARA ANDROID ESCRIBIR EN TECLADO
	...
	// METODO PARA ABRIR OTRA VENTANA
	...
	public boolean elementoPresente(By locator) {
		...
		return estado;
		// LLAMADO DEL METODO EN PAGEOBJET
		/*
		 * boolean present; try { driver.findElement(By.id("logoutLink")); present =
		 * true; } catch (NoSuchElementException e) { present = false; } Fuente:
		 * https://www.iteramos.com/pregunta/31757/webdriver-comprobar-si-un-elemento-
		 * existe
		 */
	}
	...
	// METODO PARA COMPROBAR SI UN ELEMENTO EXISTE
	public boolean elementExits(By element) 
	{ 
		driver.manage().timeouts().implicitlyWait(0, TimeUnit.MILLISECONDS);
		boolean bandera4 = driver.findElements( element ).size() != 0;
	...
	public void selectionListAndroid2(String localizador, String datoExcel, File rutacarpeta) throws Exception {

		String complet = localizador + datoExcel + "']";
		imprimirConsola(complet);
		By selectList = By.xpath(complet);

		// LLAMADO AL METODO
		boolean present = elementExits(selectList);

		while (present == false) {
			scrollVertical(rutacarpeta, 484, 1436, 135, 1);
			tiempoEspera(1);
			present = elementExits(selectList);//REVISA SI EL ELEMNTO ES TRUE O FALSE
		}
		//SI ES TRUE 
		if (present == true) {
			click(selectList, rutacarpeta);
			tiempoEspera(1);
		}

	}

	// METODO ´PARA REFRESCAR LA PAGINA EN ANDROID
